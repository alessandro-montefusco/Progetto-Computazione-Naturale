...
def pool_threads(path_params, n_client=1,P=None):
    ...
    jobs = []
    arr = []
    track_names = ["CG Speedway n.1", "Forza"]
    for i in range(n_client):
        laps = manager.dict()
        process = multiprocessing.Process(target=start_Client, args=(path_params, 3001+i, laps, track_names[i], P, True))
        arr.append(laps)
        jobs.append(process)
    for j in jobs:
        j.start()
...
def print_race_result(results):
    ...
    for res in results:
        ...
        if not len(Laps) == 0:
            for lap in Laps:
                print("Lap:", i + 1, lap.get_time(), "\t")
                print("Lap:", i + 1, lap.get_time_offtrack(), "\t")
                i = i + 1
            fscore1 = speed_fitness(Laps)
            fscore2 = speed_pos_fitness(Laps, 0, 100)
            print("fitness 1:", fscore1)
            print("fitness 2:", fscore2)
...
def start_Client(file_name, port, returnValues, track_name, P_new=None, restart=True):
    #start_server() # decommenta per runnare single_client
    T = Track()
    C = snakeoil.Client(f=file_name, p=port, P_new = P_new)
    Sensors = C.S.d
    R = C.R.d
    prev_cur_lap_time = -1
    initialize_car(C)
    ...
    for step in range(C.maxSteps, 0, -1):
        ...
        # se ha raggiunto 2 giri (caso ottimo) restart race
        if restart and len(laps) == NUMBER_OF_LAP + 1:
            #print("2 GIRI COMPLETATI - RESTARTING")
            laps.pop()
            restart_race(C)
            break

        # casi in cui la gara viene terminata dal server
        if restart and Sensors['curLapTime'] > MAXIMUM_TIME_PER_LAP:
            restart_race(C)
            laps.pop()
            #print("RESTARTING TROPPO TEMPO IMPIEGATO")
            break

        # salvataggio dati
        if (Sensors['curLapTime'] < prev_cur_lap_time):
            current_lap.set_time(prev_cur_lap_time)
            if (len(laps) <= NUMBER_OF_LAP):
                current_lap = Lap()
                laps.append(current_lap)
            else:  # numero di giri completato -> restart
                if restart:
                    print("Salvataggio - Else")
                    laps.pop()
                    restart_race(C)
                break

        #caso in cui la macchina non parte e viene prelevata dalla pista. Il tempo scorre ma non il tempo del Lap
        if (Sensors['curLapTime'] == prev_cur_lap_time):
            print("Giro mai avviato")
            laps.pop()
            restart_race(C)
            break

        current_lap.update_values(Sensors, R)
        current_lap.set_time_offtrack()
        prev_cur_lap_time = Sensors['curLapTime']

    returnValues['laps'] = laps

    with open("laps_info/sade1_forza", 'wb') as outfile:
        pickle.dump(laps, outfile)
...
if __name__ == '__main__':
    ...
    #info = pool_threads('default_parameters') #carsim
    #info = pool_threads('results/SADE/evoluzione_2/sade_fit2/best_sade_parameters_5_variant_8.txt')  #sade2
    info = pool_threads('results/PSO/evoluzione_2/Pso_fit_2/best_pso_parameters_4_variant_lbest.txt')  # pso2

    #info = pool_threads('results/PSO/evoluzione_2/Pso_fit_1/best_pso_parameters_4_variant_lbest.txt')  # pso1
    #info = pool_threads('results/SADE/evoluzione_2/sade_fit1/best_sade_parameters_4_variant_9.txt')  # sade1
    print_race_result(info)
